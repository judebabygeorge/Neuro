...
void SendDataDMA(){
	...
	nWords = 64;//Fixed Transfer

	  //Code for DMA that follows is considering a case of circular buffering
...
*/
		   SendData_DMAPARAMSET[0].A_B_CNT = (1<<16)|nWords*4;
...
//No interrupts to CPU
   ...
   // SendData_DMAPARAMSET[0].OPT =  (SendData_EvId<<12);
	SendData_DMAPARAMSET[0].SRC =  (Uint32)&SendBuffer[0] ;
	SendData_DMAPARAMSET[0].A_B_CNT = (1<<16)|0x0;

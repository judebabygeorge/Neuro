function B = CreateBarGraphDisplay(nSections,RowsPerSection,ColumnsPerSection,xlim)
        ...
        for j = 1:nRows
            for k=1:ColumnsPerSection
                for l=1:RowsPerSection          
                 ...
                 B.BarAxis{(i-1)*nRows+j , k , l}.ax=h;
                 
                 %Create Bars                 
                 y = zeros(size(x));                 
                 B.BarAxis{(i-1)*nRows+j , k , l}.bh=bar(x,y);
                 set(h,'XLim',[xl(1) xl(end)],'YLim',[0 30],'YLimMode','manual','XLimMode','manual','XTick',[],'YTick',[]); 
                 
                 %Create Text
                 B.BarAxis{(i-1)*nRows+j , k , l}.ht = text('Position',[5 15],'String','','Parent',h);  
                 
                 %Create Label
                 Pl = P;
                 Pl(2) =Pl(2) + s(2) -15;
                 Pl(4) = 15;
                 B.BarAxis{(i-1)*nRows+j , k , l}.hl = uicontrol('Style', 'text','Position', Pl,'String','hh','HorizontalAlignment','left','FontWeight','bold'); 
                end
            end
        end
    end

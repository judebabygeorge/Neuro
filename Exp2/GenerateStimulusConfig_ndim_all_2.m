function StimConfig = GenerateStimulusConfig_ndim_all_2 (Electrodes,max,ndim,PatternInterval,PulseInterval)
...
nElectrodeConfig = numel(Electrodes) +nCombinations ;
 ...
 for i=1:size(O,1)
     ...
     if(O(i,1) == O(i,2))
         ...
         delay = abs(O(i,3)-O(i,1)-1)*StimConfig.PulseInterval;
         done = 1;
     end
     ...
     if(O(i,1) == O(i,3))
         ...
         delay = abs(O(i,2)-O(i,1)-1)*StimConfig.PulseInterval;
         done = 1;
     end 
     ...
     StimConfig.PatternDelay(2,i+j*p)=StimConfig.PulseInterval+delay;
